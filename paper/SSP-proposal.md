...
**Emotion is the full-system modulation that happens in real time, driven by the registered gap between the perceived-world-graph and the should-world-graph.** The gap is data — a measurement of how far the perceived state is from the should state, computable node-by-node between the two subgraphs. Emotion is what the architecture *does* with that data: regional, global, or system-wide modulation of cognition that steers the being toward closing the gap. The gap is the signal; emotion is the response. Care, urgency, caution, satisfaction — all are modulations driven by graph-state divergences. Valence is the world-model-fit signal that drives the modulation: when the perceived graph matches the should graph, the signal is positive and modulation reinforces; when they diverge, the signal is negative and modulation redirects.
...
**Emotion is value alignment.** The full-system modulation driven by the registered gap between ideal and real, steering the architecture toward closing that gap, *is* what alignment looks like from the inside. Not a heuristic, not a proxy — the architectural substrate of alignment.
...
- **Next-state imagination** generates the immediate consequence of the next action — the WebDreamer-style baseline. It gates when the world model updates and which tier of perception to run. High prediction error against next-state escalates perception (run the expensive structural channel; trigger reconsolidation in the memory graph); low prediction error allows cheap perception (the agent does not re-ground what it correctly predicted).
- **End-state imagination** generates the *target* — a final screenshot or, more powerfully, an entire video of the action sequence — and the architecture reverse-engineers the concrete actions whose execution would reproduce that target. Two concrete instantiations: a target page screenshot whose diff against the current page is rendered as the HTML/CSS/JS that closes it; a video of a robotic task whose inverse dynamics produces the motor commands that reproduce those frames. The pipeline shifts work to where modern foundation models are strongest — imagining final states — and away from where they are weakest — grounding intermediate steps. Quality of output scales with quality of imagination, not with the density of step-by-step reasoning.

Two architectural moves complete the contribution. **Dual-channel perception** runs visual (screenshot + Set-of-Mark) and structural (DOM, accessibility tree, macOS Accessibility API) in parallel, with a *collaborative reconciler* that treats inter-channel disagreement as informative cognitive dissonance rather than as error to be suppressed. No published browser-agent framework treats inter-channel disagreement as structured information that propagates into the world model. **The Generative Collaborative reframe** re-treats the GAN-style adversarial pattern: in GRP, the imagined world and the observed world are collaborators working to converge — the being adjusts its actions and adjusts its own understanding so the observed matches the imagined. Surprise is information, not failure. This is a gravitational-attraction reframe of a classic adversarial pattern, consistent with the program's overall alignment story.
...
| I/O | Imagination-First Perception (GRP) | Imagination as perception substrate; next-state and end-state modes; collaborative dual perception |
