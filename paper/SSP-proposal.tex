...
\textbf{Emotion is the registered gap between the perceived-world-graph and the should-world-graph.} Mechanical, computable, empirically measurable. Valence is the world-model-fit signal. When the perceived graph matches the should graph, the signal is positive; when they diverge, the signal is negative. Care, urgency, caution, satisfaction --- all are graph-state divergences emitted as modulation.
...
\tightlist
\item
  \textbf{Granite --- core values.} Never change.
\item
  \textbf{Marble --- personality.} Slow evolution.
\item
  \textbf{Water --- communication register.} Adapts per context.
\item
  \textbf{Mist --- moment-to-moment state.} Constantly shifting.
\end{itemize}
...
\textbf{Emotion is value alignment.} The mechanism that measures the gap between ideal and real and modulates the entire system toward closing it \emph{is} what alignment looks like from the inside. Not a heuristic, not a proxy --- the architectural substrate of alignment.
